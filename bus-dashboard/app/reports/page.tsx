...
export default function Reports() {
  ...
  async function loadComparison() {
    ...
    (tickets || []).forEach((t) => {
      ticketMap.set(`${t.bus_number}_${t.date}`, t.ticket_count);
    });

    const merged: ComparisonRow[] = (automated || []).map((a) => {
      const key = `${a.bus_number}_${a.date}`;
      const ticketCount = ticketMap.has(key) ? ticketMap.get(key)! : null;
      return {
        bus_number: a.bus_number,
        date: a.date,
        automated_count: a.total_passengers,
        ticket_count: ticketCount,
        discrepancy: ticketCount === null ? null : a.total_passengers - ticketCount,
      };
    });
  ...
  const displayedRows = onlyFlagged
    ? rows.filter((r) => r.discrepancy !== null && r.discrepancy !== 0)
    : rows;

  function downloadCSV() {
    const headers = ["Bus", "Date", "Automated Count", "Ticket Count", "Discrepancy"];
  ...
  const flaggedCount = rows.filter((r) => r.discrepancy !== null && r.discrepancy !== 0).length;
  const missingCount = rows.filter((r) => r.ticket_count === null).length;

  return (
    <main className="min-h-screen bg-gray-50 text-gray-900 p-6">
      <h1 className="text-3xl font-bold mb-2">Discrepancy Report</h1>
      ...
      {rows.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
            <p className="text-xs text-gray-400 uppercase font-semibold">Records Compared</p>
            <p className="text-2xl font-bold">{rows.length}</p>
          </div>
          <div className="bg-white border border-red-200 rounded-2xl p-4 shadow-sm">
            <p className="text-xs text-red-400 uppercase font-semibold">Flagged Discrepancies</p>
            <p className="text-2xl font-bold text-red-500">{flaggedCount}</p>
          </div>
          <div className="bg-white border border-amber-200 rounded-2xl p-4 shadow-sm">
            <p className="text-xs text-amber-500 uppercase font-semibold">Missing Ticket Entries</p>
          ...
          <table className="w-full text-sm">
            <thead className="text-gray-500 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left py-2">Automated Count</th>
                <th className="text-left py-2">Ticket Count</th>
                <th className="text-left py-2">Discrepancy</th>
              </tr>
            </thead>
            <tbody>
              {displayedRows.map((r, i) => {
                const flagged = r.discrepancy !== null && r.discrepancy !== 0;
                return (
                  <tr
                    key={i}
                    className={`border-b border-gray-100 ${flagged ? "bg-red-50" : ""}`}
                  >
                    <td className="py-2 font-semibold" style={{ color: colorMap[r.bus_number] }}>
                      {r.bus_number}
                    </td>
                    <td className="py-2 text-gray-400">{r.date}</td>
                    <td className="py-2 font-bold">{r.automated_count}</td>
                    <td className="py-2 font-bold">
                      {r.ticket_count === null ? (
                        <span className="text-amber-500">No entry</span>
                      ) : (
                        r.ticket_count
                      )}
                    </td>
